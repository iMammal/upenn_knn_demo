...
def find_segmentation(case_dir: Path) -> Path:
    matches = sorted(case_dir.rglob("*_segm.nii.gz"))
    if not matches:
        raise FileNotFoundError(f"No *_segm.nii.gz found under {case_dir}")
    return matches[0]


def find_modality(case_dir: Path, case_id: str, modality: str) -> Path | None:
    exact = case_dir / f"{case_id}_{modality}.nii.gz"
    if exact.is_file():
        return exact
    matches = sorted(
        p
        for p in case_dir.rglob("*.nii.gz")
...
def process_case(args: argparse.Namespace) -> dict:
    ...
    seg_path = find_segmentation(case_dir)
